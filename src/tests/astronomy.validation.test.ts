 ...
 */
function runGetTimesForDate(observer: Astronomy.Observer, dateISO: string) {
 ...
 */
function assertChronologicalOrder(dates: Date[], label: string) {
...
describe('Astronomy Validation - External API Tests', () => {
  describe('Astronomical Twilight Validation (SunriseSunset.io)', () => {
    test('validates twilight times for NYC winter date', { timeout: 10000 }, async () => {
      ...
      const apiData = await fetchOrUseFixture<SunriseSunsetResponse>(
        '2024-01-15-nyc',
        () => fetchSunriseSunsetAPI(date, lat, lng, 'UTC'),
        'sunrisesunset',
        USE_FIXTURES
      );
  ...
  describe('Moon Rise/Set Validation (USNO)', () => {
    test('validates moon data for NYC', { timeout: 10000 }, async () => {
      ...
      const apiData = await fetchOrUseFixture<USNOResponse>(
        '2024-01-15-nyc',
        () => fetchUSNOAPI(date, lat, lon),
        'usno',
        USE_FIXTURES
      );
  ...
  describe('Fixture Management', () => {
    test('can save and load fixtures', { timeout: 10000 }, async () => {
      ...
      const data = await fetchOrUseFixture<SunriseSunsetResponse>(
        '2024-01-15-nyc',
        () => fetchSunriseSunsetAPI(date, lat, lng, 'UTC'),
        'sunrisesunset',
        USE_FIXTURES
  ...
  describe('Polar Region Edge Cases - Multiple Crossings', () => {
    const arcticObserver = new Astronomy.Observer(
      TEST_LOCATIONS.ARCTIC_RUSSIA.lat,
      TEST_LOCATIONS.ARCTIC_RUSSIA.lng,
      0
    );
    ...
    test('handles 3 moon crossings (2 rises, 1 set) in one day at Arctic latitude 76.5°N', () => {
      ...
      const totalCrossings = 
        (metaWindow?.meta.moonRises.length || 0) +
    ...
    test('handles multiple sun crossings of -18° threshold at Arctic latitude 76.5°N', () => {
      ...
      const totalCrossings = 
        (metaWindow?.meta.astronomicalNightStarts.length || 0) +
    ...
    test('handles polar night - sun below -18° all day at 86°N (perpetual astronomical darkness)', () => {
      ...
      const polarObserver = new Astronomy.Observer(
        TEST_LOCATIONS.POLAR_86N.lat,
        TEST_LOCATIONS.POLAR_86N.lng,
        0
      );
